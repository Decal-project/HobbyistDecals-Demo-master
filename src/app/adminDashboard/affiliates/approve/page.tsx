...
export default function ApproveAffiliatesPage() {
  ...
  useEffect(() => {
    fetch('/api/affiliate')
      .then((res) => res.json())
      .then((data) => {
        const inactive = data.filter((a: Affiliate) => !a.is_active);
        setAffiliates(inactive);
        setLoading(false);
      });
  }, []);

  const activateAffiliate = async (id: number) => {
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 p-6 overflow-y-auto">
        <h1 className="text-2xl font-bold mb-6">Pending Affiliate Approvals</h1>
