  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (numberedLine) {
      ...
      if (!numberedLine[2] && i + 1 < lines.length && lines[i + 1].trim()) {
        ...
        listItems.push(
          `<div class="flex items-start"><span class="font-bold mr-2">${numberedLine[1]}.</span><span>${applyBold(nextLine)}</span></div>`
        );
      } else {
        listItems.push(
          `<div class="flex items-start"><span class="font-bold mr-2">${numberedLine[1]}.</span><span>${applyBold(numberedLine[2])}</span></div>`
        );
      }
    } else {
      if (inList) {
...
export default function BlogDetail() {
  ...
  const recommendedBlogs = recentBlogs.filter((b) => b.id !== blog.id).sort(() => 0.5 - Math.random()).slice(0, 4);
    ...
    <>
      ...
      <div className="max-w-7xl mx-auto px-6 py-10 grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-10">
        ...
        <aside className="hidden lg:block sticky top-20 self-start h-fit">
          <div className="mb-6">
            <label htmlFor="category" className="block font-semibold mb-2">Search by Category</label>
            <div className="flex">
              <select
                id="category"
              ...
              <button className="bg-gray-800 text-white px-4 py-2 rounded-r-md" onClick={() => setSelectedCategory("")}>
                Clear
              </button>
            </div>
            ...
            {selectedCategory && (
              <div className="mt-6">
                <h3 className="text-lg font-semibold mb-2">Results in &quot;{selectedCategory}&quot;</h3>
                {filteredBlogs.length > 0 ? (
                  <ul className="space-y-2">
                    {filteredBlogs.map((blog) => (
                      <li key={blog.id}>
                        <Link href={`/blogs/${blog.id}`} className="text-gray-800 hover:text-[#16689A]">
                          {blog.title}
                        </Link>
                      </li>
                    ))}
                  ...
                  <div className="text-gray-600">No blogs found in &quot;{selectedCategory}&quot;.</div>
                )}
              </div>
            )}
          </div>

          <div className="border-t border-b py-4 text-center font-semibold tracking-wide">RECENT POSTS</div>
